...
public class NativeThreading {
	...
	public static JASSThread createThread(Callout call, VMFunctionPointer pointer) throws VMException {
		if (pointer == null)
			throw new VMUserCodeException("Cannot point to null function");
		JASSThread thread = call.machine.allocateThread("User thread", pointer);
		if (thread == null)
			throw new VMUserCodeException("Cannot create new thread");
		return thread;
	}
	...
	public static void startThread(Callout call, JASSThread thread) throws VMException {
		if (thread == null)
			throw new VMUserCodeException("Cannot provide null thread");
		if (thread.running())
			throw new VMUserCodeException("Thread already started");
		thread.runThread();
